...
from txmatching.patients.patient import Donor, Recipient, PatientType
...
def _create_recipient(recipient_id: int, donor: Donor) -> Recipient:
    return Recipient(recipient_id, f'R-{recipient_id}', related_donor=donor, parameters=PatientParameters(),
                     patient_type=PatientType.RECIPIENT, acceptable_blood_groups=list())
...
class TestMatching(TestCase):
    def setUp(self) -> None:
        self._donors = [
            Donor(donor_index, f'D-{donor_index}', parameters=PatientParameters(), patient_type=PatientType.DONOR)

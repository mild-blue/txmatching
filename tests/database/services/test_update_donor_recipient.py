...
class TestUpdateDonorRecipient(DbTests):
    ...
    def test_update_recipient(self):
        ...
        self.assertSetEqual({'0', 'A'},
                            {blood.blood_type for blood in RecipientModel.query.get(recipient_db_id).acceptable_blood})
        self.assertSetEqual({'B7', 'DQ6', 'DQ5'},
                            {hla_antibody['code']['split'] for hla_antibody_group in
                             RecipientModel.query.get(recipient_db_id).hla_antibodies['hla_antibodies_per_groups'] for
                             hla_antibody in
        ...
        self.assertSetEqual({'AB'},
                            {blood.blood_type for blood in RecipientModel.query.get(recipient_db_id).acceptable_blood})
        self.assertCountEqual([None, 'DQB1*06:03', 'DQA1*01:03'],
    ...
    def test_update_donor(self):
        ...
        self.assertSetEqual({'A11',
                             ...
                             'DR11'},
                            {hla_type['code']['split']
                             for hla_group in DonorModel.query.get(donor_db_id).hla_typing['hla_per_groups'] for
                             hla_type in
                             hla_group['hla_types']})
        ...
        self.assertSetEqual({None, 'DQB1*06:03', 'DQA1*01:03'},
                            {hla_type['code']['high_res'] for hla_group in
                             DonorModel.query.get(donor_db_id).hla_typing['hla_per_groups'] for hla_type in
                             hla_group['hla_types']})

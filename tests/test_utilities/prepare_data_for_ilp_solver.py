...
def prepare_data(txm_event_db_id: int,
                 path_to_json_file_to_export_data_to: str,
                 top_configuration_id: Optional[int] = None):
    ...
    prepared_data = {}
    bridging = [i for i, donor in enumerate(txm_event.active_donors_dict.values()) if
                donor.donor_type != DonorType.DONOR]
    prepared_data['briding'] = bridging
    ...
    with open(path_to_json_file_to_export_data_to, 'w') as f:
...
if __name__ == '__main__':
    ...
    with app.app_context():
        prepared_data = prepare_data(2, '../exported_patients_for_ilp.json')
    print(prepared_data)

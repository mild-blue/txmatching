...
def create_hla_typing(hla_types_list: List[str]) -> HLATyping:
    ...
    parsing_errors, parsed_typing_dto = parse_hla_typing_raw_and_add_parsing_error_to_db_session(hla_typing_raw=typing_dto)
    return HLATyping(
...
def create_hla_type(raw_code: str) -> HLAType:
    parsing_errors, code = parse_hla_raw_code_and_add_parsing_error_to_db_session(raw_code)
    return HLAType(
        raw_code=raw_code,
        code=code
    )
...
def create_antibodies(hla_antibodies_list: List[HLAAntibody]) -> HLAAntibodies:
    ...
    raw_antibodies = [HLAAntibodyRaw(
        ...
        mfi=hla_antibody.mfi,
        cutoff=hla_antibody.cutoff) for hla_antibody in hla_antibodies_list]
    parsing_errors, dto = parse_hla_antibodies_raw_and_add_parsing_error_to_db_session(raw_antibodies_model)

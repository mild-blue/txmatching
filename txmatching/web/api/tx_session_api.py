...
class TxSessionApi(Resource):
    ...
    @login_required()
    def post(self):
        maybe_tx_session = tx_session_service.create_tx_session(request.json["name"])
        if isinstance(maybe_tx_session, TxSession):
            return jsonify(maybe_tx_session)
        else:
            return dataclasses.asdict(maybe_tx_session), 500

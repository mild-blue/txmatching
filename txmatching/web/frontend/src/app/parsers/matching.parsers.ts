...
export const parseCalculatedMatchings = (
  data: CalculatedMatchingsGenerated,
...
export const parseMatching = (data: MatchingGenerated, patients: PatientList, listItem: ListItem): Matching => {
  const rounds = data.rounds.map((round, rKey) => parseRound(round, patients, listItem.index, rKey + 1));
  return {
    ...listItem,

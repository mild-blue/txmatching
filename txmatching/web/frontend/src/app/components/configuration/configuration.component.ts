...
export class ConfigurationComponent implements OnInit {
  ...
  public allHlaCrossmatchLevels: string[] = Object.values(HlaCrossmatchLevelGenerated);
  ...
  public submitAction(): void {
    if (this.configForm && this.configuration) {
      ...
      const { manual_donor_recipient_scores, forbidden_country_combinations, required_patient_db_ids } =
        this.configuration;
  ...
  private _buildFormFromConfig(): void {
    ...
    for (const name of names) {
      ...
      if (value !== Object(value)) {
        let validator;
        switch (name) {
          case "blood_group_compatibility_bonus":
          ...
          case "max_sequence_length":
            validator = Validators.min(0);
            break;
          case "max_number_of_distinct_countries_in_round":
            validator = Validators.min(0);
            break;
          case "max_matchings_to_show_to_viewer":
            validator = Validators.min(0);
            break;
          case "max_number_of_matchings":
            validator = [Validators.min(0), Validators.max(20)];
            break;
          case "max_debt_for_country":
            validator = Validators.min(0);
            break;
          case "max_number_of_dynamic_constrains_ilp_solver":
            validator = Validators.min(0);
            break;
          case "max_debt_for_country_for_blood_group_zero":
            validator = Validators.min(0);
            break;
          default:
            validator = undefined;
  ...
  public scorerNameToHint(scorerName: string): string {
    switch (scorerName) {
      case ScorerGenerated.SplitScorer:
        return (
          "The selected scorer SplitScorer computes transplant scores by HLA matches according to their HLA groups using compatibility index. One match in " +
      ...
      case ScorerGenerated.HighResScorer:
        return (
          "The selected scorer HighResScorer computes transplant scores by HLA matches according to their match types. Only matches corresponding " +
      ...
      case ScorerGenerated.HighResWithDqdpScorer:
        return (
          "The selected scorer HighResWithDQDPScorer computes transplant scores by HLA matches according to their match types. Only matches corresponding " +

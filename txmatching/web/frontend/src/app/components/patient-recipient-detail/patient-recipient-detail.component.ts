...
export class PatientRecipientDetailComponent extends ListItemDetailAbstractComponent implements OnInit {
  ...
  public handleSave(): void {
    ...
    this.loading = true;
    this.success = false;
    this._patientService.saveRecipient(this.defaultTxmEvent.id, this.item.dbId, this.item.updateId, this.recipientEditable)
    .then((updatedRecipient) => {
      ...
      if (updatedRecipient.parsingIssues.length > 0) {
        this._alertService.infoWithParsingIssues(
          'Recipient was updated but some parsing issues and warnings occurred. ' +
        ...
        this._logger.log('Parsing issues', updatedRecipient.parsingIssues);
      }

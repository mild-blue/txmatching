...
export class PatientService {
  ...
  public async saveDonor(
    txmEventId: number,
    donorId: number,
    donorUpdateId: number,
    ...
    const payload: DonorModelToUpdateGenerated = fromDonorEditableToUpdateGenerated(
      donorEditable,
      donorId,
      donorUpdateId
    );
    ...
    return firstValueFrom(
      this._http
        .put<UpdatedDonorGenerated>(
          `${environment.apiUrl}/txm-event/${txmEventId}/patients/configs/${configIdStr}/donor`,
          payload
        )
        .pipe(map(parseUpdatedDonor))
    );
  }

  public async saveRecipient(
    txmEventId: number,
    recipientId: number,
    recipientUpdateId: number,
    recipientEditable: RecipientEditable
  ): Promise<UpdatedRecipient> {
    this._logger.log(`Saving recipient ${recipientId}`, [recipientEditable]);
    const payload: RecipientModelToUpdateGenerated = fromRecipientEditableToUpdateGenerated(
      recipientEditable,
      recipientId,
      recipientUpdateId
    );
    ...
    return firstValueFrom(
      this._http
        .put<UpdatedRecipientGenerated>(`${environment.apiUrl}/txm-event/${txmEventId}/patients/recipient`, payload)
        .pipe(map(parseUpdatedRecipient))
    );
  }

  public async addNewPair(
    txmEventId: number,
    donor: DonorEditable,
    recipient: RecipientEditable
  ): Promise<ParsingIssuePublic[]> {
    this._logger.log("Adding new pair", [donor, recipient]);

    const payload: DonorModelPairInGenerated = fromPatientsEditableToInGenerated(donor, recipient);

    return firstValueFrom(
      this._http
        .post<PatientUploadSuccessResponseGenerated>(
          `${environment.apiUrl}/txm-event/${txmEventId}/patients/pairs`,
          payload
        )
        .pipe(map((_) => _.parsing_issues.map(parseParsingIssuePublic)))
    );
  }
  ...
  public async deleteDonor(txmEventId: number, donorDbId: number): Promise<void> {
    ...
    await firstValueFrom(
      this._http.delete(`${environment.apiUrl}/txm-event/${txmEventId}/patients/pairs/${donorDbId}`)
    );
